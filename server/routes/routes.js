...
const diskstorage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        cb(null, Date.now() + '-archivo' + file.originalname)
    }
})
    ...
    req.getConnection((err, conn) => {
        ...
        conn.query("SELECT * FROM image ", (err, rows) => {
            ...
            rows.map(img => {
                ...
                fs.writeFileSync(path.join(__dirname, "../dbimages/" + img.id + "-archivo.png"), img.data)
...
router.delete('/images/delete:id',(req, res) => {
    ...
    req.getConnection((err, conn) => {
        ...
        conn.query("DELETE * FROM image WHERE id = ?",[req.params.id], (err, rows) => {
            ...
            fs.unlinkSync(path.join(__dirname, "../dbimages/" + req.params.id + "-archivo.png"))
